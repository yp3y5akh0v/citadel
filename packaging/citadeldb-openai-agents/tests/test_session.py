...
def store(tmp_path_factory):
    ...
    return CitadelSessionStore(str(path), key="test-passphrase")
...
async def test_many_sessions_share_one_file(tmp_path):
    ...
    a = CitadelSession("user-1", path, key="pw")
    b = CitadelSession("user-2", path, key="pw")
    await a.add_items([msg("user", "a")])
    await b.add_items([msg("user", "b")])
...
async def test_search_is_semantic_and_session_scoped(store):
    ...
    await other.add_items([msg("user", "the deployment failed because the disk was full")])
...
async def test_search_finds_a_session_buried_under_another(store):
    ...
    await quiet.add_items([msg("user", "the deployment failed because the disk was full")])
...
async def test_it_survives_a_reopen(tmp_path):
    ...
    first = CitadelSessionStore(p, key="pw")
    await first.session("u1").add_items([msg("user", "the disk was full")])
    first.close()
    del first
    gc.collect()

    again = CitadelSessionStore(p, key="pw")
    s = again.session("u1")
...
async def test_a_wrong_passphrase_cannot_reopen(tmp_path):
    ...
    first = CitadelSessionStore(p, key="right")
    ...
    with pytest.raises(citadeldb.EncryptionError):
        CitadelSessionStore(p, key="wrong")
...
async def test_concurrent_turns_all_land(tmp_path):
    ...
    st = CitadelSessionStore(str(tmp_path / "conc.cdl"), key="pw")
    s = st.session("shared")
    await asyncio.gather(*(s.add_items([msg("user", f"m{i}")]) for i in range(40)))
    assert len(await s.get_items()) == 40

    await asyncio.gather(*(st.session(f"conc-{i}").add_items([msg("user", "x")])
                           for i in range(40)))
...
async def test_concurrent_pops_hand_out_each_item_once(tmp_path):
    ...
    st = CitadelSessionStore(str(tmp_path / "pop.cdl"), key="pw")
    s = st.session("shared")
...
async def test_ttl_expires_items(tmp_path):
    ...
    store = CitadelSessionStore(str(tmp_path / "ttl.cdl"), key="pw", ttl=1.0)
    s = store.session("ttl")
