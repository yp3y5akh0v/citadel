...
from citadeldb_google_adk.memory import _require_embedder
...
def svc(tmp_path_factory):
    ...
    return CitadelMemoryService(str(path), key="test-passphrase", embedder=MOCK)
...
async def test_search_on_an_empty_service_is_empty(tmp_path):
    fresh = CitadelMemoryService(str(tmp_path / "empty.cdl"), key="pw", embedder=MOCK)
...
async def test_events_can_be_added_without_a_session(svc):
    await svc.add_events_to_memory(
        app_name=APP,
        user_id=USER,
        events=[ev("delta only", eid="d-1")],
        session_id="delta",
        custom_metadata={"source": "webhook"},
    )
...
async def test_re_adding_an_event_id_replaces_it(svc):
    await svc.add_events_to_memory(
        app_name=APP,
        user_id="event-replace",
        session_id="session",
        events=[ev("first", eid="stable-event")],
    )
    await svc.add_events_to_memory(
        app_name=APP,
        user_id="event-replace",
        session_id="session",
        events=[ev("second", eid="stable-event")],
    )

    assert svc.count(APP, "event-replace") == 1
    found = await svc.search_memory(
        app_name=APP, user_id="event-replace", query="first second"
    )
...
async def test_re_adding_a_memory_id_replaces_it(svc):
    ...
    found = await svc.search_memory(
        app_name=APP, user_id="dupuser", query="first second"
    )
    assert [m.content.parts[0].text for m in found.memories] == ["second"]
...
async def test_memories_can_be_written_directly(svc):
    ...
    found = await svc.search_memory(
        app_name=APP, user_id=USER, query="written directly"
    )
...
async def test_forgetting_a_session_leaves_the_rest(svc):
    await svc.add_session_to_memory(
        sess("keep", [ev("keep me", eid="k-1")], user="carol")
    )
    await svc.add_session_to_memory(
        sess("drop", [ev("drop me", eid="k-2")], user="carol")
    )
...
async def test_it_survives_a_reopen(tmp_path):
    ...
    first = CitadelMemoryService(p, key="pw", embedder=MOCK)
    ...
    again = CitadelMemoryService(p, key="pw", embedder=MOCK)
...
def test_a_passphrase_is_required(tmp_path):
    with pytest.raises(ValueError, match="passphrase"):
        CitadelMemoryService(str(tmp_path / "nokey.cdl"), key="", embedder=MOCK)
...
async def test_concurrent_ingestion_all_lands(tmp_path):
    ...
    svc = CitadelMemoryService(str(tmp_path / "conc.cdl"), key="pw", embedder=MOCK)
...
async def test_re_adding_a_session_drops_events_removed_from_it(tmp_path):
    ...
    svc = CitadelMemoryService(str(tmp_path / "retract.cdl"), key="pw", embedder=MOCK)
...
async def test_concurrent_ingestion_of_one_session_stores_it_once(tmp_path):
    ...
    svc = CitadelMemoryService(str(tmp_path / "onesess.cdl"), key="pw", embedder=MOCK)
...
async def test_concurrent_different_session_snapshots_do_not_merge(tmp_path):
    import asyncio
    import threading

    svc = CitadelMemoryService(
        str(tmp_path / "session-set.cdl"), key="pw", embedder=MOCK
    )
    inner = svc._mem
    ...
    class CoordinatedMemory:
        ...
        def fetch(self, region, kind, **kwargs):
            criterion = kwargs.get("payload_filter") or {}
            if kind == "event" and criterion.get("sid") == "same":
                try:
    ...
    svc._mem = CoordinatedMemory()
    left = sess("same", [ev("left", eid="left")], user="race")
    right = sess("same", [ev("right", eid="right")], user="race")
    await asyncio.gather(
        svc.add_session_to_memory(left), svc.add_session_to_memory(right)
    )

    ids = {hit.payload["eid"] for hit in svc._owned(APP, "race")}
    assert ids in ({"left"}, {"right"})
...
async def test_shares_one_database_with_another_adapter(tmp_path):
    ...
    a = CitadelMemoryService(path, key="pw", region="memory", embedder=MOCK)
    b = CitadelMemoryService(path, key="pw", region="other", embedder=MOCK)
...
def test_an_embedder_is_required(tmp_path):
    with pytest.raises(TypeError, match="embedder"):
        CitadelMemoryService(str(tmp_path / "no-embedder.cdl"), key="pw")
    partial = type(
        "PartialEmbedder",
        (),
...
def test_embedder_model_id_is_normalized_without_mutating_the_caller(tmp_path):
    ...
    first = CitadelMemoryService(path, "pw", embedder=embedder)
    first._db.close()
    embedder.model_id = "stable-model"
    second = CitadelMemoryService(path, "pw", embedder=embedder)
    second._db.close()
