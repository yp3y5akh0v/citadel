...
#[test]
fn region_store_header_format_has_a_stable_known_answer() {
    let key = std::array::from_fn(|index| index as u8);
    let expected = [
        0x53, 0x4e, 0x47, 0x52, 0x01, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0a,
        0xa7, 0x07, 0x00, 0x00, 0x00, 0x2a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x56, 0x7c,
        0x02, 0xd7, 0xd2, 0x4a, 0x37, 0xc1, 0x22, 0xca, 0x31, 0xf2, 0xad, 0x95, 0xe6, 0x33, 0x3b,
        0x15, 0x4c, 0x2b, 0x9d, 0x95, 0x19, 0x72, 0xe8, 0xef, 0x5d, 0x7f, 0x30, 0x5f, 0x48, 0xaf,
    ];

    let encoded = build_header_block(&key, 0xA70A_0000_0000_0001, 7, 42);
    assert_eq!(&encoded[..HEADER_MAC_INPUT + 32], &expected);
    assert_eq!(
        parse_header_block(&key, 0xA70A_0000_0000_0001, &expected),
        Some((7, 42))
    );
}

#[test]
fn key_store_slot_format_has_a_stable_known_answer() {
    let key = std::array::from_fn(|index| index as u8);
    let wrapped = std::array::from_fn(|index| index as u8);
    let expected = [
        0x01, 0x00, 0x00, 0x00, 0x08, 0x07, 0x06, 0x05, 0x04, 0x03, 0x02, 0x01, 0x88, 0x77, 0x66,
        0x55, 0x44, 0x33, 0x22, 0x11, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09,
        0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18,
        0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27,
        0x86, 0xb4, 0xe6, 0xae, 0xba, 0x31, 0xfd, 0x62, 0xf9, 0x11, 0x51, 0x0d, 0xce, 0x57, 0xf1,
        0xdf, 0xfe, 0x68, 0xaa, 0x4e, 0xf4, 0x22, 0x04, 0x74, 0x8e, 0x8c, 0x0e, 0x59, 0xab, 0xe1,
        0xb1, 0x32,
    ];

    let encoded = build_slot_block(
        &key,
        SlotState::Live,
    ...
    assert_eq!(&encoded[..SLOT_MAC_INPUT + 32], &expected);

    let decoded = parse_slot_block(&key, &expected).unwrap();
    assert_eq!(decoded.state, SlotState::Live);
    assert_eq!(decoded.region_id, 0x0102_0304_0506_0708);
    assert_eq!(decoded.gen, 0x1122_3344_5566_7788);
    assert_eq!(decoded.wrapped, wrapped);
}
