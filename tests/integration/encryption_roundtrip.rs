...
#[test]
fn sieve_eviction_under_pressure() {
    use citadel_buffer::sieve::SieveCache;

    let mut cache = SieveCache::<String>::new(5);
    ...
    // Access entries 0 and 4 (mark visited)
    cache.get(0);
    cache.get(4);
    ...
    assert_ne!(ek, 0, "visited entry should survive");
    assert_ne!(ek, 2, "dirty entry should survive");
    assert_ne!(ek, 4, "visited entry should survive");
}
