...
fn has_any_window_function(stmt: &SelectStmt) -> bool {
...
/// Extract window functions from an expression, replacing them with column references.
/// Returns the rewritten expression and a list of (slot_name, window_fn_name, args, spec).
fn extract_window_fns(
    expr: &Expr,
...
fn resolve_frame(spec: &WindowSpec) -> WindowFrame {
...
fn rows_frame_indices(frame: &WindowFrame, i: usize, n: usize) -> Result<(usize, usize)> {
...
impl MonoDeque {
    ...
    fn push(&mut self, idx: usize, val: Value) {
        if val.is_null() {
            return;
        }
    ...
    fn pop_expired(&mut self, frame_start: usize) {
...
impl SlidingSum {
    ...
    fn result_count(&self) -> Value {
        Value::Integer(self.count)
    }

    fn result_avg(&self) -> Value {
        if self.count == 0 {
            Value::Null
        } else {
...
fn eval_window_select(
    ...
    if all_extracted.is_empty() {
        return process_select(columns, rows, stmt, false);
    }
    ...
    // 3. Group window functions by (partition_by, order_by) spec for sort sharing
    // We'll process each window function in the order extracted, sorting as needed
    // Track row identity across sorts via index mapping
    let n = rows.len();
    ...
    for (win_idx, (_, fn_name, _, spec)) in all_extracted.iter().enumerate() {
        ...
        for &(ps, pe) in &partitions {
            ...
            // Build a ColumnMap for the partition rows (for frame_indices RANGE mode)
            // We'll create a virtual "partition rows" slice for ORDER BY key evaluation
    ...
    process_select(&extended_columns, rows, &rewritten_stmt, true)
}
