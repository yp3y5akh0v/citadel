...
SQL engine for the [Citadel](https://github.com/yp3y5akh0v/citadel) encrypted embedded database engine. Supports CREATE/DROP TABLE, SELECT with JOINs, subqueries, CTEs, window functions, INSERT (with UPSERT / `ON CONFLICT`), UPDATE, DELETE, RETURNING (with `OLD`/`NEW` row aliases), indexes, aggregates, constraints, transactions with savepoints, prepared statements, and multi-statement scripts.
