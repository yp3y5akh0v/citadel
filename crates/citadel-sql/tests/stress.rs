...
fn insert_1000_rows_verify_all() {
    ...
    // Verify each row
    for i in 0..1000i64 {
        let qr = conn.query(&format!("SELECT val FROM t WHERE id = {i}")).unwrap();
        assert_eq!(qr.rows.len(), 1, "row {i} missing");
        assert_eq!(qr.rows[0][0], Value::Integer(i * 7), "wrong value for row {i}");
    }
}
...
fn insert_then_delete_half_verify_remaining() {
    ...
    for i in 0..500 {
        let qr = conn.query(&format!("SELECT id FROM t WHERE id = {i}")).unwrap();
        if i % 2 == 0 {
...
fn many_sequential_inserts() {
    ...
    conn.execute(
        "CREATE TABLE t (id INTEGER NOT NULL PRIMARY KEY, batch INTEGER)"
    ).unwrap();
    ...
    for batch in 0..50 {
        for _ in 0..10 {
            conn.execute(&format!("INSERT INTO t VALUES ({id}, {batch})")).unwrap();
            id += 1;
        }
    }
    ...
    for batch in 0..50 {
        let qr = conn.query(&format!(
            "SELECT COUNT(*) FROM t WHERE batch = {batch}"
        )).unwrap();
...
fn create_drop_cycle_50_times() {
    ...
    for i in 0..50 {
        ...
        conn.execute(&format!(
            "INSERT INTO {name} VALUES (1, 'round_{i}')"
        )).unwrap();

        let qr = conn.query(&format!("SELECT val FROM {name} WHERE id = 1")).unwrap();
...
fn recreate_same_table_with_different_schema() {
    ...
    conn.execute(
        "CREATE TABLE t (id INTEGER NOT NULL PRIMARY KEY, score REAL, active BOOLEAN)"
    ).unwrap();
    conn.execute("INSERT INTO t VALUES (1, 99.5, TRUE)").unwrap();

    let qr = conn.query("SELECT score, active FROM t WHERE id = 1").unwrap();
...
fn create_50_tables_all_active() {
    ...
    for i in 0..50 {
        ...
        conn.execute(&format!(
            "INSERT INTO t_{i} VALUES ({i}, {})", i * 100
        )).unwrap();
    }
    ...
    for i in 0..50 {
        let qr = conn.query(&format!("SELECT val FROM t_{i} WHERE id = {i}")).unwrap();
...
fn repeated_open_close_cycles() {
    ...
    for cycle in 2..=21 {
        ...
        let qr = conn.query("SELECT COUNT(*) FROM t").unwrap();
        assert_eq!(qr.rows[0][0], Value::Integer(cycle - 1),
            "wrong count at cycle {cycle}");

        conn.execute(&format!("INSERT INTO t VALUES ({cycle}, {})", cycle * 10)).unwrap();
    }
...
fn write_close_reopen_verify_10_cycles() {
...

    {
        ...
        let mut conn = Connection::open(&db).unwrap();
        conn.execute(
            "CREATE TABLE t (id INTEGER NOT NULL PRIMARY KEY, data TEXT)"
        ).unwrap();
    }
    ...
    for cycle in 0..10 {
        ...
        // Reopen and verify
        {
            ...
            let qr = conn.query("SELECT COUNT(*) FROM t").unwrap();
            assert_eq!(qr.rows[0][0], Value::Integer(expected_count as i64),
...
fn delete_all_reinsert_cycle() {
    ...
    conn.execute(
        "CREATE TABLE t (id INTEGER NOT NULL PRIMARY KEY, gen INTEGER)"
    ).unwrap();

    for gen in 0..10 {
        // Insert 50 rows
        for i in 0..50 {
            conn.execute(&format!("INSERT INTO t VALUES ({i}, {gen})")).unwrap();
        }
...
fn large_text_values() {
    ...
    for i in 1..=20 {
        ...
        conn.execute(&format!("INSERT INTO t VALUES ({i}, '{text}')")).unwrap();
    }

    // Verify each
    for i in 1..=20 {
        let size = i * 25;
        let qr = conn.query(&format!("SELECT data FROM t WHERE id = {i}")).unwrap();
        match &qr.rows[0][0] {
...
fn value_near_max_inline_size() {
    ...
    conn.execute(&format!("INSERT INTO t VALUES (1, '{text}')")).unwrap();
...
fn value_exceeding_max_inline_size_is_rejected() {
    ...
    assert!(result.is_err(), "value exceeding max inline size should be rejected");
}
...
fn table_with_many_nullable_columns() {
    ...
    conn.execute(
        "INSERT INTO sparse (id, nullable_5, nullable_10) VALUES (2, 50, 100)"
    ).unwrap();

    let qr = conn.query("SELECT nullable_5, nullable_10 FROM sparse WHERE id = 2").unwrap();
    assert_eq!(qr.rows[0][0], Value::Integer(50));
...
fn order_by_500_rows_ascending() {
    ...
    for i in (0..500).rev() {
        conn.execute(&format!("INSERT INTO t VALUES ({i}, {})", 500 - i)).unwrap();
    }
...
fn order_by_text_500_rows() {
    ...
    for i in 0..500 {
        ...
        conn.execute(&format!("INSERT INTO t VALUES ({i}, '{name}')")).unwrap();
    }
...
fn group_by_100_groups() {
    ...
    for grp in 0..100 {
        for val in 0..5 {
            conn.execute(&format!("INSERT INTO t VALUES ({id}, {grp}, {val})")).unwrap();
            id += 1;
        }
    }
...
fn interleaved_insert_update_delete() {
    ...
    for round in 0..20 {
        ...
        for j in 0..10 {
            let id = base + j;
            conn.execute(&format!("INSERT INTO t VALUES ({id}, 0)")).unwrap();
        }

        // Update the first 5
        for j in 0..5 {
            let id = base + j;
            conn.execute(&format!("UPDATE t SET val = 1 WHERE id = {id}")).unwrap();
        }
...
fn schema_persists_with_multiple_tables_across_reopen() {
...

    {
        let db = create_db(dir.path());
        let mut conn = Connection::open(&db).unwrap();
        conn.execute("CREATE TABLE users (id INTEGER NOT NULL PRIMARY KEY, name TEXT)").unwrap();
        conn.execute("CREATE TABLE posts (id INTEGER NOT NULL PRIMARY KEY, user_id INTEGER, body TEXT)").unwrap();
        conn.execute("CREATE TABLE tags (id INTEGER NOT NULL PRIMARY KEY, label TEXT NOT NULL)").unwrap();
...

    {
        ...
        conn.execute("CREATE TABLE comments (id INTEGER NOT NULL PRIMARY KEY, text TEXT)").unwrap();
        conn.execute("INSERT INTO comments VALUES (1, 'Nice!')").unwrap();
    }

    {
        ...
        let qr = conn.query("SELECT text FROM comments WHERE id = 1").unwrap();
...
fn all_types_in_one_table() {
    ...
    let qr = conn.query("SELECT COUNT(*) FROM mixed WHERE int_col >= 5000").unwrap();
    assert_eq!(qr.rows[0][0], Value::Integer(50));

    let qr = conn.query("SELECT COUNT(*) FROM mixed WHERE real_col > 5.0").unwrap();
    assert_eq!(qr.rows[0][0], Value::Integer(49)); // 51..99 -> 49 rows (5.1, 5.2, ..., 9.9)

    let qr = conn.query("SELECT COUNT(*) FROM mixed WHERE bool_col = TRUE").unwrap();
    assert_eq!(qr.rows[0][0], Value::Integer(50));
    ...
    let qr = conn.query("SELECT MIN(text_col), MAX(text_col) FROM mixed").unwrap();
...
fn update_all_then_reopen() {
...

    {
        ...
        for (i, row) in qr.rows.iter().enumerate() {
            assert_eq!(row[0], Value::Integer(i as i64));
            assert_eq!(row[1], Value::Integer(i as i64 * 2),
                "wrong val for id {i}");
        }
    }
}
...
fn rapid_queries_500() {
    ...
    for i in 0..50 {
        conn.execute(&format!("INSERT INTO t VALUES ({i}, {})", i * i)).unwrap();
    }
