...
fn persistence_across_reopen() {
    ...
    // Create and populate
    {
        ...
        let mut conn = Connection::open(&db).unwrap();
        conn.execute(
            "CREATE TABLE items (id INTEGER NOT NULL PRIMARY KEY, label TEXT NOT NULL)"
        ).unwrap();
        conn.execute("INSERT INTO items (id, label) VALUES (1, 'alpha')").unwrap();
        conn.execute("INSERT INTO items (id, label) VALUES (2, 'beta')").unwrap();
        conn.execute("INSERT INTO items (id, label) VALUES (3, 'gamma')").unwrap();
    }
...
fn multi_table_isolation() {
    ...
    conn.execute(
        "CREATE TABLE orders (id INTEGER NOT NULL PRIMARY KEY, product_id INTEGER)"
    ).unwrap();

    conn.execute("INSERT INTO products (id, name) VALUES (1, 'Widget')").unwrap();
    conn.execute("INSERT INTO products (id, name) VALUES (2, 'Gadget')").unwrap();
    conn.execute("INSERT INTO orders (id, product_id) VALUES (100, 1)").unwrap();
...
fn order_by_ascending_descending() {
    ...
    conn.execute(
        "CREATE TABLE scores (id INTEGER NOT NULL PRIMARY KEY, score INTEGER)"
    ).unwrap();
    ...
    // ASC
    let qr = conn.query("SELECT id, score FROM scores ORDER BY score ASC").unwrap();
    assert_eq!(qr.rows[0][1], Value::Integer(70));
    assert_eq!(qr.rows[3][1], Value::Integer(95));

    // DESC
    let qr = conn.query("SELECT id, score FROM scores ORDER BY score DESC").unwrap();
...
fn limit_and_offset() {
    ...
    conn.execute(
        "CREATE TABLE nums (id INTEGER NOT NULL PRIMARY KEY)"
    ).unwrap();
    for i in 1..=10 {
        conn.execute(&format!("INSERT INTO nums VALUES ({i})")).unwrap();
    }

    let qr = conn.query("SELECT id FROM nums ORDER BY id LIMIT 3").unwrap();
    assert_eq!(qr.rows.len(), 3);
    assert_eq!(qr.rows[0][0], Value::Integer(1));
    assert_eq!(qr.rows[2][0], Value::Integer(3));

    let qr = conn.query("SELECT id FROM nums ORDER BY id LIMIT 3 OFFSET 5").unwrap();
    assert_eq!(qr.rows.len(), 3);
    assert_eq!(qr.rows[0][0], Value::Integer(6));
    assert_eq!(qr.rows[2][0], Value::Integer(8));

    // OFFSET past end
    let qr = conn.query("SELECT id FROM nums ORDER BY id OFFSET 100").unwrap();
...
fn aggregate_functions() {
    ...
    conn.execute("INSERT INTO sales VALUES (1, 100.0, 'east')").unwrap();
    conn.execute("INSERT INTO sales VALUES (2, 200.0, 'west')").unwrap();
    conn.execute("INSERT INTO sales VALUES (3, 150.0, 'east')").unwrap();
    conn.execute("INSERT INTO sales VALUES (4, 300.0, 'west')").unwrap();
    conn.execute("INSERT INTO sales VALUES (5, 50.0, 'east')").unwrap();
    ...
    // MIN / MAX
    let qr = conn.query("SELECT MIN(amount), MAX(amount) FROM sales").unwrap();
...
fn group_by_having() {
    ...
    conn.execute("INSERT INTO sales VALUES (1, 100, 'east')").unwrap();
    conn.execute("INSERT INTO sales VALUES (2, 200, 'west')").unwrap();
    conn.execute("INSERT INTO sales VALUES (3, 150, 'east')").unwrap();
    conn.execute("INSERT INTO sales VALUES (4, 300, 'west')").unwrap();

    let qr = conn.query(
        "SELECT region, SUM(amount) AS total FROM sales GROUP BY region HAVING total > 300"
    ).unwrap();
...
fn null_handling() {
    ...
    conn.execute("INSERT INTO data VALUES (1, 'hello')").unwrap();
    conn.execute("INSERT INTO data VALUES (2, NULL)").unwrap();
    conn.execute("INSERT INTO data VALUES (3, 'world')").unwrap();
    ...
    // IS NOT NULL
    let qr = conn.query("SELECT id FROM data WHERE val IS NOT NULL").unwrap();
    assert_eq!(qr.rows.len(), 2);
...
fn boolean_type() {
    ...
    let qr = conn.query("SELECT id FROM flags WHERE active = TRUE").unwrap();
    assert_eq!(qr.rows.len(), 2);

    let qr = conn.query("SELECT COUNT(*) FROM flags WHERE active = FALSE").unwrap();
    assert_eq!(qr.rows[0][0], Value::Integer(1));
}
...
fn complex_where() {
    ...
    conn.execute("INSERT INTO items VALUES (1, 10.0, 'food')").unwrap();
    conn.execute("INSERT INTO items VALUES (2, 50.0, 'electronics')").unwrap();
    conn.execute("INSERT INTO items VALUES (3, 5.0, 'food')").unwrap();
    conn.execute("INSERT INTO items VALUES (4, 100.0, 'electronics')").unwrap();
    conn.execute("INSERT INTO items VALUES (5, 25.0, 'clothing')").unwrap();

    // AND
    let qr = conn.query(
        "SELECT id FROM items WHERE category = 'food' AND price > 7.0"
    ).unwrap();
    assert_eq!(qr.rows.len(), 1);
    assert_eq!(qr.rows[0][0], Value::Integer(1));
    ...
    // OR
    let qr = conn.query(
        "SELECT id FROM items WHERE category = 'food' OR category = 'clothing' ORDER BY id"
    ).unwrap();
    assert_eq!(qr.rows.len(), 3);

    // Arithmetic in WHERE
    let qr = conn.query(
        "SELECT id FROM items WHERE price * 2 > 90.0 ORDER BY id"
    ).unwrap();
    assert_eq!(qr.rows.len(), 2);
    assert_eq!(qr.rows[0][0], Value::Integer(2));
...
fn update_multiple_columns() {
    ...
    conn.execute(
        "CREATE TABLE users (id INTEGER NOT NULL PRIMARY KEY, name TEXT, score INTEGER)"
    ).unwrap();
    conn.execute("INSERT INTO users VALUES (1, 'Alice', 100)").unwrap();

    assert_rows_affected(conn.execute(
        "UPDATE users SET name = 'Alicia', score = 200 WHERE id = 1"
    ).unwrap(), 1);

    let qr = conn.query("SELECT name, score FROM users WHERE id = 1").unwrap();
...
fn update_no_matches() {
    ...
    assert_rows_affected(conn.execute(
        "UPDATE t SET v = 99 WHERE id = 999"
    ).unwrap(), 0);
}
...
fn real_precision() {
    ...
    conn.execute("INSERT INTO t VALUES (1, 3.14159265358979)").unwrap();

    let qr = conn.query("SELECT val FROM t WHERE id = 1").unwrap();
    match &qr.rows[0][0] {
        Value::Real(r) => assert!((*r - 3.14159265358979).abs() < 1e-12),
...
fn case_insensitive_identifiers() {
    ...
    conn.execute(
        "CREATE TABLE MyTable (ID INTEGER NOT NULL PRIMARY KEY, Name TEXT)"
    ).unwrap();
    conn.execute("INSERT INTO MYTABLE (id, name) VALUES (1, 'test')").unwrap();
...
fn insert_many_rows() {
    ...
    for i in 0..100 {
        conn.execute(&format!("INSERT INTO t VALUES ({i}, {0})", i * 10)).unwrap();
    }
    ...
    let qr = conn.query("SELECT id FROM t ORDER BY id DESC LIMIT 1").unwrap();
...
fn select_with_expression_in_projection() {
    ...
    conn.execute(
        "CREATE TABLE t (id INTEGER NOT NULL PRIMARY KEY, price REAL, qty INTEGER)"
    ).unwrap();
    conn.execute("INSERT INTO t VALUES (1, 10.5, 3)").unwrap();

    let qr = conn.query("SELECT price * qty FROM t WHERE id = 1").unwrap();
    match &qr.rows[0][0] {
...
fn composite_primary_key() {
    ...
    conn.execute("INSERT INTO t VALUES (1, 1, 'one-one')").unwrap();
    conn.execute("INSERT INTO t VALUES (1, 2, 'one-two')").unwrap();
    conn.execute("INSERT INTO t VALUES (2, 1, 'two-one')").unwrap();

    let qr = conn.query("SELECT val FROM t WHERE a = 1 ORDER BY b").unwrap();
    assert_eq!(qr.rows.len(), 2);
...
fn update_with_expression() {
    ...
    conn.execute(
        "CREATE TABLE t (id INTEGER NOT NULL PRIMARY KEY, score INTEGER)"
    ).unwrap();
    conn.execute("INSERT INTO t VALUES (1, 100)").unwrap();

    conn.execute("UPDATE t SET score = score + 50 WHERE id = 1").unwrap();
...
fn tables_listing() {
    ...
    conn.execute("CREATE TABLE alpha (id INTEGER NOT NULL PRIMARY KEY)").unwrap();
    conn.execute("CREATE TABLE beta (id INTEGER NOT NULL PRIMARY KEY)").unwrap();
    conn.execute("CREATE TABLE gamma (id INTEGER NOT NULL PRIMARY KEY)").unwrap();
