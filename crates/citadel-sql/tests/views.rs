...
fn create_simple_view() {
    ...
    assert_ok(conn.execute("CREATE VIEW all_users AS SELECT * FROM users").unwrap());
...
fn create_view_with_column_aliases() {
    ...
    assert_ok(conn.execute(
        "CREATE VIEW user_info (user_id, user_name) AS SELECT id, name FROM users"
    ).unwrap());
...
fn create_view_if_not_exists_existing() {
    ...
    assert_ok(conn.execute("CREATE VIEW IF NOT EXISTS v AS SELECT id FROM users").unwrap());
...
fn create_view_or_replace_existing() {
    ...
    assert_ok(conn.execute("CREATE VIEW v AS SELECT * FROM users").unwrap());
    assert_ok(conn.execute("CREATE OR REPLACE VIEW v AS SELECT id, name FROM users").unwrap());
...
fn create_view_case_insensitive() {
    ...
    assert_ok(conn.execute("CREATE VIEW MyView AS SELECT * FROM users").unwrap());
...
fn create_view_with_where() {
    ...
    assert_ok(conn.execute(
        "CREATE VIEW adults AS SELECT * FROM users WHERE age >= 30"
    ).unwrap());
...
fn create_view_with_distinct() {
    ...
    assert_ok(conn.execute(
        "CREATE VIEW distinct_ages AS SELECT DISTINCT age FROM users"
    ).unwrap());

    let qr = conn.query("SELECT * FROM distinct_ages ORDER BY age").unwrap();
...
fn create_view_with_expressions() {
    ...
    let qr = conn.query("SELECT label FROM user_labels WHERE id = 1").unwrap();
...
fn create_view_on_nonexistent_table_lazy() {
    ...
    assert_ok(conn.execute("CREATE VIEW v AS SELECT * FROM ghost").unwrap());
...
fn create_view_same_name_as_table() {
    ...
    let err = conn.execute("CREATE VIEW users AS SELECT * FROM users").unwrap_err();
...
fn create_table_same_name_as_view() {
    ...
    let err = conn.execute(
        "CREATE TABLE v (id INTEGER NOT NULL PRIMARY KEY)"
    ).unwrap_err();
...
fn create_duplicate_view() {
    ...
    assert_ok(conn.execute("CREATE VIEW v AS SELECT * FROM users").unwrap());
    let err = conn.execute("CREATE VIEW v AS SELECT id FROM users").unwrap_err();
    assert!(matches!(err, SqlError::ViewAlreadyExists(_)));
}
...
fn drop_and_recreate_view() {
    ...
    assert_ok(conn.execute("CREATE VIEW v AS SELECT * FROM users").unwrap());
    assert_ok(conn.execute("DROP VIEW v").unwrap());
    assert_ok(conn.execute("CREATE VIEW v AS SELECT id FROM users").unwrap());
...
fn select_with_limit_offset_on_view() {
    ...
    let qr = conn.query("SELECT * FROM v ORDER BY id LIMIT 1 OFFSET 1").unwrap();
    assert_eq!(qr.rows.len(), 1);
...
fn select_with_group_by_on_view() {
    ...
    let qr = conn.query("SELECT age, COUNT(*) FROM v GROUP BY age ORDER BY age").unwrap();
    assert_eq!(qr.rows.len(), 3);
}
...
fn select_distinct_on_view() {
    ...
    let qr = conn.query("SELECT DISTINCT age FROM v ORDER BY age").unwrap();
    assert_eq!(qr.rows.len(), 3);
}
...
fn view_inner_join_table() {
    ...
    assert_rows_affected(conn.execute(
        "INSERT INTO orders (id, user_id, total) VALUES (1, 1, 50.0)"
    ).unwrap(), 1);

    assert_ok(conn.execute("CREATE VIEW v AS SELECT * FROM users").unwrap());

    let qr = conn.query(
        "SELECT v.name, orders.total FROM v INNER JOIN orders ON v.id = orders.user_id"
    ).unwrap();
    assert_eq!(qr.rows.len(), 1);
    assert_eq!(qr.rows[0][0], Value::Text("Alice".into()));
}
...
fn table_left_join_view() {
    ...
    assert_rows_affected(conn.execute(
        "INSERT INTO scores (id, user_id, score) VALUES (1, 1, 100)"
    ).unwrap(), 1);

    assert_ok(conn.execute("CREATE VIEW v AS SELECT * FROM scores").unwrap());
...
fn view_join_view() {
    ...
    assert_ok(conn.execute("CREATE VIEW vu AS SELECT * FROM users").unwrap());
    assert_ok(conn.execute("CREATE VIEW vo AS SELECT * FROM orders").unwrap());

    let qr = conn.query(
        "SELECT vu.name, vo.total FROM vu INNER JOIN vo ON vu.id = vo.user_id"
    ).unwrap();
    assert_eq!(qr.rows.len(), 1);
}
...
fn view_in_in_subquery() {
    ...
    let qr = conn.query(
        "SELECT name FROM users WHERE id IN (SELECT id FROM adults)"
    ).unwrap();
    assert_eq!(qr.rows.len(), 2);
}
...
fn view_in_exists_subquery() {
    ...
    let qr = conn.query(
        "SELECT name FROM users WHERE EXISTS (SELECT 1 FROM adults)"
    ).unwrap();
    assert_eq!(qr.rows.len(), 3);

    // NOT EXISTS with empty view result
    assert_ok(conn.execute(
        "CREATE VIEW centenarians AS SELECT * FROM users WHERE age >= 100"
    ).unwrap());
    let qr = conn.query(
        "SELECT name FROM users WHERE NOT EXISTS (SELECT 1 FROM centenarians)"
    ).unwrap();
    assert_eq!(qr.rows.len(), 3);
}
...
fn view_in_scalar_subquery() {
    ...
    let qr = conn.query("SELECT (SELECT cnt FROM adult_count) AS n").unwrap();
    assert_eq!(qr.rows[0][0], Value::Integer(2));
}
...
fn view_in_insert_select() {
    ...
    assert_rows_affected(conn.execute(
        "INSERT INTO archive SELECT * FROM adults"
    ).unwrap(), 2);
...
fn nested_view_two_levels() {
    ...
    assert_ok(conn.execute("CREATE VIEW v1 AS SELECT * FROM users WHERE age >= 25").unwrap());
    assert_ok(conn.execute("CREATE VIEW v2 AS SELECT * FROM v1 WHERE age <= 35").unwrap());
...
fn nested_view_three_levels() {
    ...
    assert_ok(conn.execute("CREATE VIEW v1 AS SELECT * FROM users").unwrap());
    assert_ok(conn.execute("CREATE VIEW v2 AS SELECT * FROM v1 WHERE age >= 25").unwrap());
    assert_ok(conn.execute("CREATE VIEW v3 AS SELECT name FROM v2 WHERE age <= 30").unwrap());
...
fn circular_view_detection() {
    ...
    assert_ok(conn.execute("CREATE VIEW va AS SELECT * FROM users").unwrap());
    // Replace va to reference vb
    assert_ok(conn.execute("CREATE VIEW vb AS SELECT * FROM va").unwrap());
    assert_ok(conn.execute("CREATE OR REPLACE VIEW va AS SELECT * FROM vb").unwrap());
...
fn self_referencing_view() {
    ...
    assert_ok(conn.execute("CREATE OR REPLACE VIEW v AS SELECT * FROM v").unwrap());
...
fn insert_into_view_error() {
    ...
    let err = conn.execute(
        "INSERT INTO v (id, name, email, age) VALUES (4, 'Dave', 'd@t.com', 40)"
    ).unwrap_err();
...
fn update_view_error() {
    ...
    let err = conn.execute("UPDATE v SET age = 99 WHERE id = 1").unwrap_err();
    assert!(matches!(err, SqlError::CannotModifyView(_)));
}
...
fn multiple_views_persist() {
    let dir = tempfile::tempdir().unwrap();
    {
        ...
        let mut conn = Connection::open(&db).unwrap();
        setup_users(&mut conn);
        assert_ok(conn.execute("CREATE VIEW v1 AS SELECT * FROM users WHERE age < 30").unwrap());
        assert_ok(conn.execute("CREATE VIEW v2 AS SELECT * FROM users WHERE age >= 30").unwrap());
    }
    {
        let db = open_db(dir.path());
        let mut conn = Connection::open(&db).unwrap();
...
fn create_table_and_view_in_txn() {
    ...
    assert_rows_affected(conn.execute("INSERT INTO t (id, val) VALUES (1, 42)").unwrap(), 1);
...
fn cte_body_references_view() {
    ...
    let qr = conn.query(
        "WITH filtered AS (SELECT * FROM v WHERE name = 'Alice') SELECT * FROM filtered"
    ).unwrap();
    assert_eq!(qr.rows.len(), 1);
...
fn cte_same_name_as_view_takes_precedence() {
    ...
    let qr = conn.query(
        "WITH v AS (SELECT 42 AS val) SELECT * FROM v"
    ).unwrap();
    assert_eq!(qr.rows.len(), 1);
...
fn simple_view_fuses_with_outer_where() {
    ...
    assert_ok(conn.execute("CREATE VIEW v AS SELECT * FROM users WHERE age >= 25").unwrap());
