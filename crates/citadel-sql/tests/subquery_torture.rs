...
fn setup_tables(conn: &mut Connection) {
    assert_ok(conn.execute(
        "CREATE TABLE t1 (id INTEGER NOT NULL PRIMARY KEY, val INTEGER)"
    ).unwrap());
    assert_ok(conn.execute(
        "CREATE TABLE t2 (id INTEGER NOT NULL PRIMARY KEY, val INTEGER)"
    ).unwrap());
    assert_rows_affected(conn.execute(
        "INSERT INTO t1 (id, val) VALUES (1, 10), (2, 20), (3, 30), (4, 40), (5, 50)"
    ).unwrap(), 5);
    assert_rows_affected(conn.execute(
        "INSERT INTO t2 (id, val) VALUES (2, 200), (4, 400), (6, 600)"
    ).unwrap(), 3);
}

fn get_ids(qr: &QueryResult) -> Vec<i64> {
    let mut ids: Vec<i64> = qr.rows.iter()
        .map(|r| match &r[0] { Value::Integer(i) => *i, other => panic!("expected int, got {other:?}") })
        .collect();
    ids.sort();
    ids
}
...
fn in_subquery_duplicate_values_in_subquery() {
    ...
    assert_ok(conn.execute(
        "CREATE TABLE b (id INTEGER NOT NULL PRIMARY KEY, ref_id INTEGER NOT NULL)"
    ).unwrap());
    assert_rows_affected(conn.execute(
        "INSERT INTO a (id) VALUES (1), (2), (3)"
    ).unwrap(), 3);
    assert_rows_affected(conn.execute(
        "INSERT INTO b (id, ref_id) VALUES (1, 1), (2, 1), (3, 2), (4, 2)"
    ).unwrap(), 4);

    let qr = query(&mut conn, "SELECT id FROM a WHERE id IN (SELECT ref_id FROM b)");
...
fn in_subquery_with_text_values() {
    ...
    assert_ok(conn.execute(
        "CREATE TABLE allowed (id INTEGER NOT NULL PRIMARY KEY, name TEXT NOT NULL)"
    ).unwrap());
    assert_rows_affected(conn.execute(
        "INSERT INTO t (id, name) VALUES (1, 'Alice'), (2, 'Bob'), (3, 'Charlie')"
    ).unwrap(), 3);
    assert_rows_affected(conn.execute(
        "INSERT INTO allowed (id, name) VALUES (1, 'Alice'), (2, 'Charlie')"
    ).unwrap(), 2);
...
fn scalar_subquery_null_comparison() {
    ...
    let qr = query(&mut conn,
        "SELECT id FROM t WHERE val = (SELECT id FROM s)"
    );
    assert_eq!(qr.rows.len(), 0);
}
...
fn delete_with_exists() {
    ...
    assert_rows_affected(conn.execute(
        "DELETE FROM t1 WHERE EXISTS (SELECT 1 FROM t2 WHERE t2.id = 999)"
    ).unwrap(), 0);
...
fn update_set_with_scalar_subquery() {
    ...
    assert_rows_affected(conn.execute(
        "UPDATE t1 SET val = (SELECT MIN(val) FROM t2)"
    ).unwrap(), 5);
...
fn update_where_exists() {
    ...
    assert_rows_affected(conn.execute(
        "UPDATE t1 SET val = 0 WHERE EXISTS (SELECT 1 FROM t2)"
    ).unwrap(), 5);
...
fn update_where_not_exists() {
    ...
    assert_rows_affected(conn.execute(
        "UPDATE t1 SET val = 0 WHERE NOT EXISTS (SELECT 1 FROM t2)"
    ).unwrap(), 0);
}
...
fn in_subquery_sees_uncommitted_data() {
    ...
    assert_rows_affected(conn.execute(
        "INSERT INTO t2 (id, val) VALUES (1, 100)"
    ).unwrap(), 1);
...
fn update_in_subquery_within_transaction() {
    ...
    conn.execute("BEGIN").unwrap();
    assert_rows_affected(conn.execute(
        "UPDATE t1 SET val = 999 WHERE id IN (SELECT id FROM t2)"
    ).unwrap(), 2);
...
fn in_subquery_scale() {
    ...
    assert_ok(conn.execute(
        "CREATE TABLE filter (id INTEGER NOT NULL PRIMARY KEY)"
    ).unwrap());

    conn.execute("BEGIN").unwrap();
    for i in 0..200 {
        conn.execute(&format!("INSERT INTO big (id, val) VALUES ({i}, {val})", val = i * 10)).unwrap();
    }
    for i in (0..200).step_by(3) {
        conn.execute(&format!("INSERT INTO filter (id) VALUES ({i})")).unwrap();
    }
    conn.execute("COMMIT").unwrap();
...
fn in_with_boolean_column() {
    ...
    assert_ok(conn.execute(
        "CREATE TABLE flags (id INTEGER NOT NULL PRIMARY KEY, flag BOOLEAN NOT NULL)"
    ).unwrap());
    assert_rows_affected(conn.execute(
        "INSERT INTO t (id, active) VALUES (1, TRUE), (2, FALSE), (3, TRUE)"
    ).unwrap(), 3);
    assert_rows_affected(conn.execute(
        "INSERT INTO flags (id, flag) VALUES (1, TRUE)"
    ).unwrap(), 1);
...
fn having_with_scalar_subquery() {
    ...
    assert_rows_affected(conn.execute(
        "INSERT INTO thresholds (id, min_total) VALUES (1, 100)"
    ).unwrap(), 1);
...
fn in_subquery_with_real_values() {
    ...
    assert_ok(conn.execute(
        "CREATE TABLE prices (id INTEGER NOT NULL PRIMARY KEY, price REAL NOT NULL)"
    ).unwrap());
    assert_ok(conn.execute(
        "CREATE TABLE discounts (id INTEGER NOT NULL PRIMARY KEY, price REAL NOT NULL)"
    ).unwrap());
    assert_rows_affected(conn.execute(
        "INSERT INTO prices (id, price) VALUES (1, 9.99), (2, 19.99), (3, 29.99)"
    ).unwrap(), 3);
    assert_rows_affected(conn.execute(
        "INSERT INTO discounts (id, price) VALUES (1, 9.99), (2, 29.99)"
    ).unwrap(), 2);
