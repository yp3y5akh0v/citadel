...
fn in_subquery_basic() {
    ...
    let qr = query(&mut conn, "SELECT id FROM t1 WHERE id IN (SELECT id FROM t2)");
...
fn not_in_subquery_basic() {
    ...
    let qr = query(&mut conn, "SELECT id FROM t1 WHERE id NOT IN (SELECT id FROM t2)");
    let mut ids: Vec<i64> = qr.rows.iter()
        .map(|r| match &r[0] { Value::Integer(i) => *i, _ => panic!() })
        .collect();
    ids.sort();
...
fn in_subquery_with_null_in_subquery_result() {
    ...
    assert_ok(conn.execute(
        "CREATE TABLE items (id INTEGER NOT NULL PRIMARY KEY, cat INTEGER)"
    ).unwrap());
    assert_ok(conn.execute(
        "CREATE TABLE cats (id INTEGER NOT NULL PRIMARY KEY, cat_id INTEGER)"
    ).unwrap());
    assert_rows_affected(conn.execute(
        "INSERT INTO items (id, cat) VALUES (1, 10), (2, 20), (3, 30)"
    ).unwrap(), 3);
    assert_rows_affected(conn.execute(
        "INSERT INTO cats (id, cat_id) VALUES (1, 10), (2, NULL), (3, 30)"
    ).unwrap(), 3);
...
fn not_in_subquery_with_null_returns_zero_rows() {
    ...
    assert_ok(conn.execute(
        "CREATE TABLE b (id INTEGER NOT NULL PRIMARY KEY, val INTEGER)"
    ).unwrap());
    assert_rows_affected(conn.execute(
        "INSERT INTO a (id) VALUES (1), (2), (3)"
    ).unwrap(), 3);
    assert_rows_affected(conn.execute(
        "INSERT INTO b (id, val) VALUES (1, 10), (2, NULL)"
    ).unwrap(), 2);
...
fn null_lhs_in_subquery() {
    ...
    assert_rows_affected(conn.execute(
        "INSERT INTO t (id, val) VALUES (1, NULL), (2, 10)"
    ).unwrap(), 2);
    assert_rows_affected(conn.execute(
        "INSERT INTO s (id) VALUES (10), (20)"
    ).unwrap(), 2);

    let qr = query(&mut conn,
        "SELECT id FROM t WHERE val IN (SELECT id FROM s)"
    );
    let ids: Vec<i64> = qr.rows.iter()
        .map(|r| match &r[0] { Value::Integer(i) => *i, _ => panic!() })
        .collect();
    assert_eq!(ids, vec![2]);
}
...
fn not_in_all_null_subquery() {
    ...
    assert_ok(conn.execute(
        "CREATE TABLE s (id INTEGER NOT NULL PRIMARY KEY, val INTEGER)"
    ).unwrap());
    assert_rows_affected(conn.execute(
        "INSERT INTO t (id) VALUES (1), (2)"
    ).unwrap(), 2);
    assert_rows_affected(conn.execute(
        "INSERT INTO s (id, val) VALUES (1, NULL), (2, NULL)"
    ).unwrap(), 2);
...
fn scalar_subquery_multiple_rows_error() {
    ...
    let result = conn.execute(
        "SELECT id FROM t1 WHERE val = (SELECT val FROM t2)"
    );
...
fn in_subquery_multiple_columns_error() {
    ...
    let result = conn.execute(
        "SELECT id FROM t1 WHERE id IN (SELECT id, val FROM t2)"
    );
...
fn subquery_table_not_found() {
    ...
    let result = conn.execute(
        "SELECT id FROM t1 WHERE id IN (SELECT id FROM nonexistent)"
    );
...
fn in_subquery_with_group_by_having() {
    ...
    assert_ok(conn.execute(
        "CREATE TABLE vip_products (id INTEGER NOT NULL PRIMARY KEY)"
    ).unwrap());
    assert_rows_affected(conn.execute(
        "INSERT INTO orders (id, product, qty) VALUES (1, 1, 5), (2, 1, 10), (3, 2, 3), (4, 3, 7)"
    ).unwrap(), 4);
    assert_rows_affected(conn.execute(
        "INSERT INTO vip_products (id) VALUES (1), (2)"
    ).unwrap(), 2);
...
fn in_subquery_with_join() {
    ...
    assert_ok(conn.execute(
        "CREATE TABLE vip (id INTEGER NOT NULL PRIMARY KEY)"
    ).unwrap());
    assert_rows_affected(conn.execute(
...
fn update_with_scalar_subquery() {
    ...
    assert_rows_affected(conn.execute(
        "UPDATE t1 SET val = (SELECT MAX(val) FROM t2) WHERE id = 1"
    ).unwrap(), 1);
...
fn update_where_in_subquery() {
    ...
    let qr = query(&mut conn, "SELECT id, val FROM t1 WHERE val = 999 ORDER BY id");
