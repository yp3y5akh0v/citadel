...
fn basic_inner_join() {
    ...
    assert_eq!(qr.rows[0], vec![Value::Text("Alice".into()), Value::Real(50.0)]);
    assert_eq!(qr.rows[1], vec![Value::Text("Alice".into()), Value::Real(30.0)]);
    assert_eq!(qr.rows[2], vec![Value::Text("Bob".into()), Value::Real(100.0)]);
}
...
fn cross_join_cartesian_product() {
    ...
    assert_ok(conn.execute(
        "CREATE TABLE b (y INTEGER NOT NULL PRIMARY KEY)"
    ).unwrap());
    assert_rows_affected(conn.execute("INSERT INTO a (x) VALUES (1), (2)").unwrap(), 2);
    assert_rows_affected(conn.execute("INSERT INTO b (y) VALUES (10), (20), (30)").unwrap(), 3);

    let qr = query(&mut conn, "SELECT a.x, b.y FROM a CROSS JOIN b ORDER BY a.x, b.y");
...
fn self_join() {
    ...
    assert_eq!(qr.rows.len(), 2);
    assert_eq!(qr.rows[0], vec![Value::Text("Alice".into()), Value::Text("Boss".into())]);
    assert_eq!(qr.rows[1], vec![Value::Text("Bob".into()), Value::Text("Boss".into())]);
}
...
fn three_table_join() {
    ...
    assert_rows_affected(conn.execute(
        "INSERT INTO products (id, title) VALUES (100, 'Widget'), (200, 'Gadget')"
    ).unwrap(), 2);
    assert_rows_affected(conn.execute(
    ...
    assert_eq!(qr.rows.len(), 3);
    assert_eq!(qr.rows[0], vec![Value::Text("Alice".into()), Value::Text("Gadget".into())]);
    assert_eq!(qr.rows[1], vec![Value::Text("Alice".into()), Value::Text("Widget".into())]);
    assert_eq!(qr.rows[2], vec![Value::Text("Bob".into()), Value::Text("Widget".into())]);
}
...
fn join_on_non_pk_column() {
    ...
    assert_rows_affected(conn.execute(
        "INSERT INTO t2 (id, val) VALUES (100, 10), (200, 30)"
    ).unwrap(), 2);
...
fn join_null_equality_excludes_rows() {
    ...
    assert_ok(conn.execute(
        "CREATE TABLE t1 (id INTEGER NOT NULL PRIMARY KEY, ref_id INTEGER)"
    ).unwrap());
    assert_ok(conn.execute(
        "CREATE TABLE t2 (id INTEGER NOT NULL PRIMARY KEY)"
    ).unwrap());
    assert_rows_affected(conn.execute(
        "INSERT INTO t1 (id, ref_id) VALUES (1, NULL), (2, 100)"
    ).unwrap(), 2);
    assert_rows_affected(conn.execute(
...
fn join_with_complex_on_clause() {
    ...
    assert_ok(conn.execute(
        "CREATE TABLE t1 (id INTEGER NOT NULL PRIMARY KEY, x INTEGER NOT NULL)"
    ).unwrap());
    assert_ok(conn.execute(
        "CREATE TABLE t2 (id INTEGER NOT NULL PRIMARY KEY, y INTEGER NOT NULL)"
    ).unwrap());
    assert_rows_affected(conn.execute(
        "INSERT INTO t1 (id, x) VALUES (1, 10), (2, 20), (3, 5)"
    ).unwrap(), 3);
    assert_rows_affected(conn.execute(
        "INSERT INTO t2 (id, y) VALUES (100, 10), (200, 20)"
    ).unwrap(), 2);
...
fn join_same_table_twice() {
    ...
    assert_eq!(qr.rows[1], vec![Value::Text("A".into()), Value::Text("C".into())]);
    assert_eq!(qr.rows[2], vec![Value::Text("B".into()), Value::Text("C".into())]);
}
...
fn join_scale_100x1000() {
    ...
    for i in 0..1000 {
        let uid = i % 100;
        conn.execute(&format!(
            "INSERT INTO orders (id, uid, amt) VALUES ({i}, {uid}, {})", i * 10
        )).unwrap();
    }
...
fn join_within_transaction() {
    ...
    assert_rows_affected(conn.execute(
        "INSERT INTO t1 (id, val) VALUES (1, 'x'), (2, 'y')"
    ).unwrap(), 2);
    assert_rows_affected(conn.execute(
        "INSERT INTO t2 (id, ref_id) VALUES (10, 1), (20, 2)"
    ).unwrap(), 2);
    ...
    assert_eq!(qr.rows.len(), 2);
    assert_eq!(qr.rows[0], vec![Value::Text("x".into()), Value::Integer(10)]);
...
fn right_join_includes_unmatched_right() {
    ...
    assert_eq!(qr.rows[1], vec![Value::Text("Alice".into()), Value::Integer(11)]);
    assert_eq!(qr.rows[2], vec![Value::Text("Bob".into()), Value::Integer(12)]);
}
...
fn right_join_null_pads_left_side() {
    ...
    assert_rows_affected(conn.execute(
        "INSERT INTO t2 (id, ref_id) VALUES (10, 1), (20, 999)"
    ).unwrap(), 2);
    ...
    assert_eq!(qr.rows.len(), 2);
    assert_eq!(qr.rows[0], vec![Value::Text("A".into()), Value::Integer(10)]);
...
fn right_join_within_transaction() {
    ...
    assert_rows_affected(conn.execute(
        "INSERT INTO t1 (id, val) VALUES (1, 'x')"
    ).unwrap(), 1);
    assert_rows_affected(conn.execute(
        "INSERT INTO t2 (id, ref_id) VALUES (10, 1), (20, 999)"
    ).unwrap(), 2);
...
fn join_table_not_found() {
    ...
    let result = conn.execute(
        "SELECT * FROM t1 JOIN nonexistent ON t1.id = nonexistent.id"
    );
